...
# (Sequence length 100, Batch 4096, LR 0.002, WITH ProbSparse Attention)
# ============================================================================
import glob
import torch
import numpy as np
...
# ============================================================================
DATA_FOLDER = '/projects/tc6d/attn_resid/1_stack_prob/Data'
...
PROBSPARSE_TOP_K = 10 # Number of active queries for ProbSparse attention
...
class ProbSparseAttention(nn.Module):
    """Simplified ProbSparse Self-Attention for temporal sequences.
    
    Based on the Informer paper, this attention mechanism exploits the long-tailed
    distribution of self-attention scores by computing full attention only for 
    the most active (sparse) queries, reducing complexity from O(L²) to O(L log L).
    """
    def __init__(self, hidden_size, num_heads=4, top_k=25, dropout=0.1):
        super().__init__()
        self.hidden_size = hidden_size
        self.num_heads = num_heads
        self.top_k = top_k  # number of active queries to keep
    ...
    def _sparsity_measure(self, queries, keys):
        """
        Compute sparsity score for each query using max-mean measurement.
        ...
        Returns:
            sparsity: (B, L) - higher values indicate more informative queries
        """
        B, L, D = queries.shape
        
        # Sample a subset of keys for efficiency
        num_sample = min(50, keys.size(1))
        
        # Use deterministic sampling for reproducibility
        if keys.size(1) > num_sample:
            sample_step = keys.size(1) // num_sample
            sample_idx = torch.arange(0, keys.size(1), sample_step, device=keys.device)[:num_sample]
        else:
            sample_idx = torch.arange(keys.size(1), device=keys.device)
        
        sample_keys = keys[:, sample_idx, :]  # (B, sample, D)
        
        # Compute dot products with sampled keys
        scores = torch.matmul(queries, sample_keys.transpose(-2, -1))  # (B, L, sample)
        scores = scores / (self.hidden_size ** 0.25)
        
        # Max-mean measurement from Informer paper
        max_score = scores.max(dim=-1)[0]  # (B, L)
        mean_score = scores.mean(dim=-1)    # (B, L)
        sparsity = max_score - mean_score   # (B, L)
    ...
    def forward(self, hidden_seq):
        """
        Args:
            hidden_seq: (B, L, D) - sequence of hidden states
        ...
        # Compute sparsity scores for each query position
        sparsity = self._sparsity_measure(hidden_seq, hidden_seq)  # (B, L)
        
        # Select top-k most active queries
        actual_k = min(self.top_k, L)
        _, top_indices = torch.topk(sparsity, actual_k, dim=-1)  # (B, top_k)
        
        # Create sparse query: gather top-k positions
        batch_indices = torch.arange(B, device=hidden_seq.device).unsqueeze(1).expand(-1, actual_k)
        sparse_queries = hidden_seq[batch_indices, top_indices, :]  # (B, top_k, D)
        
        # Full attention with sparse queries against all keys/values
        attn_out, _ = self.attention(
            query=sparse_queries,
            key=hidden_seq,
            value=hidden_seq
        )  # (B, top_k, D)
        
        # Residual connection and normalization
        sparse_output = sparse_queries + self.dropout(attn_out)
        sparse_output = self.norm(sparse_output)
        
        # Pool to single context vector using mean of active queries
        context = sparse_output.mean(dim=1)  # (B, D)
        
        return context

class ImprovedxLSTMPredictor(nn.Module):
    """N-Layer Stacked xLSTM with ProbSparse Attention readout."""
    def __init__(self, input_size=1, hidden_size=128, mem_dim=16, output_size=1, 
                 num_layers=4, attention_iterations=3, top_k=25):
        super().__init__()
        self.input_size = input_size
        self.hidden_size = hidden_size
        self.mem_dim = mem_dim
        ...
        # ProbSparse Attention readout (replaces simple last-timestep)
        self.readout = ProbSparseAttention(
            hidden_size=hidden_size // 2,  # 64
            num_heads=2,
            top_k=top_k,
            dropout=0.1
        )
    ...
    def init_hidden(self, batch_size, device):
        ...
        for i in range(self.num_layers):
            h = torch.zeros(batch_size, self.hidden_size // 2, 
                          device=device, dtype=torch.float16)
            m = torch.zeros(batch_size, self.mem_dim, self.mem_dim, 
                          device=device, dtype=torch.float16)
        ...
        for i in range(self.num_layers):
            h = torch.zeros(batch_size, self.hidden_size // 2, 
                          device=device, dtype=torch.float16)
            c = torch.zeros(batch_size, self.hidden_size // 2, 
                          device=device, dtype=torch.float16)
    ...
    def forward(self, x):
        ...
        # Collect all hidden states for attention readout
        all_hidden = []
        ...
        for t in range(seq_len):
            ...
            # Store the output at this timestep
            all_hidden.append(x_proj)
        
        # Stack all timesteps and apply ProbSparse attention readout
        hidden_seq = torch.stack(all_hidden, dim=1)  # (B, seq_len, hidden//2)
        context = self.readout(hidden_seq)            # (B, hidden//2)
...
def save_checkpoint(epoch, model, optimizer, scheduler, train_losses, val_losses, 
    ...
    checkpoint = {
        ...
        'config': {
            ...
            'PROBSPARSE_TOP_K': PROBSPARSE_TOP_K,
...
if __name__ == "__main__":
    ...
    print("\n" + "="*70)
    print(f" xLSTM VIBRATION PREDICTION - {NUM_LAYERS}-LAYER STACK (ProbSparse Attention) (BS={BATCH_SIZE})")
    print("="*70)
    print(f"\nConfiguration:")
    print(f"  Sequence Length: {SEQUENCE_LENGTH}")
    ...
    print(f"  ProbSparse Top-K: {PROBSPARSE_TOP_K}")
    ...
    model = ImprovedxLSTMPredictor(
        ...
        output_size=1,
        num_layers=NUM_LAYERS,
        attention_iterations=ATTENTION_ITERATIONS,  # kept for compatibility
        top_k=PROBSPARSE_TOP_K
    )
    ...
    if not args.deepspeed or args.local_rank == 0:
        ...
        save_dict = {
            ...
            'config': {
                ...
                'memory_dim': MEMORY_DIM,
                'num_layers': NUM_LAYERS,
                'prob_sparse_top_k': PROBSPARSE_TOP_K,
                'batch_size': BATCH_SIZE,
                'learning_rate': LEARNING_RATE,
        ...
        ax4.plot([y_actual.min(), y_actual.max()], [y_actual.min(), y_actual.max()], 
 ...
 Hidden: {HIDDEN_SIZE}
 Memory: {MEMORY_DIM}
 Top-K:  {PROBSPARSE_TOP_K}
 Batch:  {BATCH_SIZE}
 LR:     {LEARNING_RATE}
 """
        ax6.text(0.1, 0.5, metrics_text, fontsize=9, family='monospace',
                verticalalignment='center')
        ax6.set_title(f'{NUM_LAYERS}-Layer xLSTM (ProbSparse Attention)', 
