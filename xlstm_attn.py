...
# (Sequence length 100, Batch 4096, LR 0.002, Iterative Attention Reasoner)
# ============================================================================ 
import glob
import torch
import numpy as np
...
# ============================================================================ 
DATA_FOLDER = '/projects/tc6d/attn_resid/1_stack/Data'  
...
ATTENTION_ITERATIONS = 3            # number of reasoning iterations
...
class IterativeAttentionReasoner(nn.Module):
    """Iterative attention reasoner that refines a context vector over the sequence."""
    def __init__(self, hidden_size, num_iterations=3, num_heads=4):
        super().__init__()
    ...
    def forward(self, hidden_seq):
        # hidden_seq: (batch, seq_len, hidden_size)
        c = hidden_seq[:, -1:, :]  # initial query = last hidden state

        for _ in range(self.num_iterations):
            attn_out, _ = self.attention(query=c,
                                         key=hidden_seq,
                                         value=hidden_seq)
            c = self.norm1(c + attn_out)

            ffn_out = self.ffn(c)
            c = self.norm2(c + ffn_out)

        return c.squeeze(1)  # (batch, hidden_size)

class ImprovedxLSTMPredictor(nn.Module):
    """N-Layer Stacked xLSTM with Iterative Attention Reasoner."""
    def __init__(self, input_size=1, hidden_size=128, mem_dim=16,
        ...
        # Iterative Attention Reasoner
        self.reasoner = IterativeAttentionReasoner(hidden_size // 2,
                                                   num_iterations=attention_iterations,
                                                   num_heads=4)
    ...
    def forward(self, x):
        ...
        all_hidden = []  # store per‑step outputs for the reasoner
        ...
        for t in range(seq_len):
            ...
            all_hidden.append(x_proj)  # shape: (batch, hidden//2)

        hidden_seq = torch.stack(all_hidden, dim=1)   # (batch, seq_len, hidden//2)
        context = self.reasoner(hidden_seq)            # (batch, hidden//2)
...
if __name__ == "__main__":
    ...
    print("\n" + "="*70)
    print(f"  xLSTM VIBRATION PREDICTION - {NUM_LAYERS}-LAYER STACK + ATTENTION REASONER (BS={BATCH_SIZE})")
    print("="*70)
    print(f"\nConfiguration:")
    print(f"  Sequence Length: {SEQUENCE_LENGTH}")
    ...
    print(f"  Attention Iter:  {ATTENTION_ITERATIONS}")
    ...
    if not args.deepspeed or args.local_rank == 0:
        ...
        ax4.plot([y_actual.min(), y_actual.max()], [y_actual.min(), y_actual.max()], 'r--', lw=2, label='Perfect')
        ...
        metrics_text = f"""
         ...
         Model: {NUM_LAYERS}-Layer xLSTM + Attn Reasoner
         Hidden: {HIDDEN_SIZE}
         Memory: {MEMORY_DIM}
        ...
        ax6.set_title(f'{NUM_LAYERS}-Layer xLSTM + Attention Reasoner', fontsize=11, fontweight='bold')
