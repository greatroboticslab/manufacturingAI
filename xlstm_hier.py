# ============================================================================ 
# xLSTM VIBRATION PREDICTION - HIERARCHICAL CHUNK-WISE WITH ATTENTION
# (Sequence length 100, Chunk size 20, 5 chunks, Batch 4096, LR 0.002)
# ============================================================================ 
import glob
import torch
import numpy as np
...
# ============================================================================
DATA_FOLDER = '/projects/tc6d/attn_resid/1_stack_hier/Data'
...
HIDDEN_SIZE = 128
MEMORY_DIM = 16
NUM_LAYERS = 1  # Number of xLSTM layers within each chunk
CHUNK_SIZE = 20  # Size of each chunk
NUM_CHUNKS = SEQUENCE_LENGTH // CHUNK_SIZE  # 5 chunks
ATTENTION_HEADS = 2  # Number of attention heads for chunk aggregation
...
class ChunkAttentionAggregator(nn.Module):
    """Attention-based aggregation of chunk representations"""
    def __init__(self, hidden_size, num_heads=2, num_chunks=5):
        super().__init__()
        self.hidden_size = hidden_size
        ...
        self.pos_encoding = nn.Parameter(
            torch.randn(1, num_chunks, hidden_size) * 0.02
        )
        
        # Layer norms and feed-forward for transformer block
        ...
        # Learnable query for cross-attention (to get single context vector)
        self.context_query = nn.Parameter(torch.randn(1, 1, hidden_size) * 0.02)
        
        self._init_weights()
    
    def _init_weights(self):
        nn.init.xavier_uniform_(self.ffn[0].weight, gain=0.5)
        nn.init.constant_(self.ffn[0].bias, 0.0)
        nn.init.xavier_uniform_(self.ffn[3].weight, gain=0.5)
        nn.init.constant_(self.ffn[3].bias, 0.0)
    
    def forward(self, chunk_repr):
        """
        chunk_repr: (B, num_chunks, hidden_size)
        Returns: (B, hidden_size) - aggregated context
        """
        B, num_chunks, hidden_size = chunk_repr.shape
        
        # Add positional encoding
        chunk_repr = chunk_repr + self.pos_encoding
        
        # Self-attention over chunks
        attn_out, attn_weights = self.attention(chunk_repr, chunk_repr, chunk_repr)
        chunk_repr = self.norm1(chunk_repr + attn_out)
        
        # Feed-forward with residual
        ffn_out = self.ffn(chunk_repr)
        chunk_repr = self.norm2(chunk_repr + ffn_out)
        
        # Cross-attention with learnable query to get single context vector
        context_query = self.context_query.expand(B, -1, -1)
        context, _ = self.attention(context_query, chunk_repr, chunk_repr)
        context = context.squeeze(1)  # (B, hidden_size)
        
        return context

class ImprovedxLSTMPredictor(nn.Module):
    """Hierarchical Chunk-wise xLSTM with Attention Aggregation"""
    def __init__(self, input_size=1, hidden_size=128, mem_dim=16, output_size=1, 
                 num_layers=1, chunk_size=20, num_chunks=5, attention_heads=2):
        super().__init__()
        self.input_size = input_size
        self.hidden_size = hidden_size
        self.mem_dim = mem_dim
        ...
        # Reduced hidden size for xLSTM within chunks (to save memory)
        chunk_hidden_size = hidden_size // 2
        
        # Shared xLSTM blocks for processing each chunk
        self.mlstm_layers = nn.ModuleList([
            ImprovedmLSTMBlock(
                hidden_size if i == 0 else chunk_hidden_size,
                chunk_hidden_size,
                mem_dim=mem_dim
        ...
        self.slstm_layers = nn.ModuleList([
            ImprovedsLSTMBlock(chunk_hidden_size, chunk_hidden_size)
            for i in range(num_layers)
        ])
        
        # Layer norms and dropouts between layers within chunks
        num_transitions = num_layers * 2 - 1
        self.layer_norms = nn.ModuleList([
            nn.LayerNorm(chunk_hidden_size) for _ in range(num_transitions)
        ])
        self.dropouts = nn.ModuleList([
            nn.Dropout(0.1) for _ in range(num_transitions)
        ])
        
        # Attention aggregator for chunks
        self.chunk_aggregator = ChunkAttentionAggregator(
            hidden_size=chunk_hidden_size,
            num_heads=attention_heads,
            num_chunks=num_chunks
        )
        
        # Output layer
        self.output_layer = nn.Linear(chunk_hidden_size, output_size)
    ...
    def init_hidden(self, batch_size, device):
        ...
        hidden_states = []
        chunk_hidden_size = self.hidden_size // 2
        
        # mLSTM layers
        for i in range(self.num_layers):
            h = torch.zeros(batch_size, chunk_hidden_size, device=device, dtype=torch.float16)
            m = torch.zeros(batch_size, self.mem_dim, self.mem_dim, device=device, dtype=torch.float16)
            hidden_states.append((h, m))
        
        # sLSTM layers
        for i in range(self.num_layers):
            h = torch.zeros(batch_size, chunk_hidden_size, device=device, dtype=torch.float16)
            c = torch.zeros(batch_size, chunk_hidden_size, device=device, dtype=torch.float16)
            hidden_states.append((h, c))
        
        return hidden_states
    
    def process_chunk(self, chunk_input, hidden_states):
        """
        Process a single chunk through the xLSTM layers
        chunk_input: (B, chunk_size, input_dim)
        hidden_states: initial hidden states
        Returns: final hidden state (B, hidden_size)
        """
        B, chunk_size, _ = chunk_input.shape
        
        # Initial projection
        x_proj = torch.tanh(self.input_proj(chunk_input)) * 0.5  # (B, chunk_size, hidden_size)
        
        # Initialize fresh hidden states for this chunk
        hidden = self.init_hidden(B, chunk_input.device)
        
        # Process through time steps
        for t in range(chunk_size):
            x_t = x_proj[:, t, :]  # (B, hidden_size)
            
            # Process through mLSTM layers
            for i in range(self.num_layers):
                x_t, hidden[i] = self.mlstm_layers[i](x_t, hidden[i])
                if i < self.num_layers - 1:
                    x_t = self.layer_norms[i](x_t)
                    x_t = self.dropouts[i](x_t)
            
            # Process through sLSTM layers
            for i in range(self.num_layers):
                x_t, hidden[self.num_layers + i] = self.slstm_layers[i](
                    x_t, hidden[self.num_layers + i]
                )
                if i < self.num_layers - 1:
                    idx = self.num_layers - 1 + i
                    x_t = self.layer_norms[idx](x_t)
                    x_t = self.dropouts[idx](x_t)
        
        # Return the final hidden state (after last sLSTM)
        final_hidden = hidden[-1][0]  # (B, chunk_hidden_size)
        return final_hidden
    ...
    def forward(self, x):
        ...
        2. Process each chunk with xLSTM
        3. Aggregate chunks with attention
        4. Predict output
        """
        B, L, _ = x.shape
        
        # Ensure sequence length matches expected
        assert L == self.chunk_size * self.num_chunks, \
            f"Sequence length {L} must equal chunk_size * num_chunks = {self.chunk_size * self.num_chunks}"
        
        # Reshape into chunks: (B, num_chunks, chunk_size, input_size)
        x_chunks = x.view(B, self.num_chunks, self.chunk_size, self.input_size)
        
        # Process each chunk independently
        chunk_outputs = []
        for chunk_idx in range(self.num_chunks):
            chunk_input = x_chunks[:, chunk_idx, :, :]  # (B, chunk_size, input_size)
            chunk_repr = self.process_chunk(chunk_input, None)  # (B, chunk_hidden_size)
            chunk_outputs.append(chunk_repr)
        
        # Stack chunk representations: (B, num_chunks, chunk_hidden_size)
        chunk_repr = torch.stack(chunk_outputs, dim=1)
        
        # Aggregate chunks with attention
        context = self.chunk_aggregator(chunk_repr)  # (B, chunk_hidden_size)
...
if __name__ == "__main__":
    ...
    print("\n" + "="*70)
    print(f" xLSTM VIBRATION PREDICTION - HIERARCHICAL CHUNK-WISE WITH ATTENTION")
    print(f" (Seq={SEQUENCE_LENGTH}, Chunk={CHUNK_SIZE}, Chunks={NUM_CHUNKS}, BS={BATCH_SIZE})")
    print("="*70)
    print(f"\nConfiguration:")
    print(f"  Sequence Length: {SEQUENCE_LENGTH}")
    ...
    print(f"  xLSTM Layers per Chunk: {NUM_LAYERS}")
    ...
    if not args.deepspeed or args.local_rank == 0:
        ...
        Model: Hierarchical Chunk-wise
        xLSTM + Attention
        Chunks: {NUM_CHUNKS} × {CHUNK_SIZE} steps
        xLSTM Layers: {NUM_LAYERS}
        ...
        ax6.set_title('Hierarchical Chunk-wise xLSTM', fontsize=11, fontweight='bold')
