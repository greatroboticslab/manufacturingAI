...
# ============================================================================
DATA_FOLDER = '/projects/tc6d/xLSTM_1_Stack/Data'
...
HIDDEN_SIZE = 128
MEMORY_DIM = 16
NUM_LAYERS = 1   # ← Change this to 1, 2, 3, 4, or 5 to control stack depth
...
SEQUENCE_STEP = 1
USE_MIXED_PRECISION = True  # Handled by DeepSpeed config
...
# ============================================================================
# MODEL ARCHITECTURE - FLEXIBLE N-LAYER STACK (set NUM_LAYERS above)
# ============================================================================
...
class ImprovedxLSTMPredictor(nn.Module):
    """N-Layer Stacked xLSTM — set num_layers to 1, 2, 3, 4, or 5"""
    def __init__(self, input_size=1, hidden_size=128, mem_dim=16, output_size=1, num_layers=4):
        super().__init__()
        self.input_size = input_size
        self.hidden_size = hidden_size
        self.mem_dim = mem_dim
        ...
        self.mlstm_layers = nn.ModuleList([
            ImprovedmLSTMBlock(hidden_size if i == 0 else hidden_size // 2, hidden_size // 2, mem_dim=mem_dim)
        ...
        # Layer norms and dropouts between layers (num_layers*2 - 1 transitions)
    ...
    def init_hidden(self, batch_size, device):
        """Initialize hidden states for all N layers"""
        hidden_states = []
...
def save_history_to_txt(epoch, val_loss, val_r2, loss_file, accuracy_file, args):
    """Append validation loss and accuracy to text files for plotting"""
    if not args.deepspeed or args.local_rank == 0:
...
def calculate_r2_score(actual, predicted):
    """Calculate R² score (used as accuracy metric for regression)"""
...
if __name__ == "__main__":
    ...
    print("\n" + "="*70)
    print(" xLSTM VIBRATION PREDICTION - {}-LAYER STACK WITH DEEPSPEED".format(NUM_LAYERS))
    print("="*70)
    print(f"\nConfiguration:")
    print(f"  Sequence Length: {SEQUENCE_LENGTH}")
    print(f"  Hidden Size: {HIDDEN_SIZE}")
    print(f"  Memory Dim: {MEMORY_DIM}")
    print(f"  Stacking: {NUM_LAYERS} mLSTM layers + {NUM_LAYERS} sLSTM layers")
    ...
    if args.deepspeed and args.deepspeed_config:
        ...
        # Note: do NOT pass training_data here — we manage DataLoaders ourselves
    ...
    # ==================================================
    # DATA LOADERS — created AFTER DeepSpeed init
    # so DistributedSampler knows the correct rank/world_size
    # ==================================================
    if args.deepspeed:
    ...
    # ==================================================
    # CHECKPOINT LOADING - RESUME FROM PREVIOUS STATE
    # ==================================================
    print(f"\n{'='*70}")
    ...
    for epoch in range(start_epoch, NUM_EPOCHS):
        ...
        for batch_X, batch_y in progress_bar:
            ...
            # DeepSpeed manages its own gradient zeroing — only call for non-DeepSpeed
        ...
        # Save metrics and print on main process only
        if not args.deepspeed or args.local_rank == 0:
    ...
    # ==================================================
    # FINAL RESULTS — main process only
    # ==================================================
    if not args.deepspeed or args.local_rank == 0:
        print(f"\n{'='*70}")
...
Memory: {MEMORY_DIM}
"""
        ax6.text(0.1, 0.5, metrics_text, fontsize=10, family='monospace', verticalalignment='center')
        ax6.set_title(f'{NUM_LAYERS}-Layer Stacked xLSTM', fontsize=11, fontweight='bold')
