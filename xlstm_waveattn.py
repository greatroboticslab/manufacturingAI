...
# Optional: Wavelet Attention Front-End (toggle USE_WAVELET_ATTENTION)
# ============================================================================
import glob
import torch
import numpy as np
...
# ============================================================================
DATA_FOLDER = '/projects/tc6d/attn_resid/1_stack_wave/Data'
...
# Model parameters (ORIGINAL - unchanged)
SEQUENCE_LENGTH = 100
HIDDEN_SIZE = 128
MEMORY_DIM = 16
...
# Wavelet Attention toggle
USE_WAVELET_ATTENTION = True  # Set to True to enable wavelet attention front-end

# Wavelet parameters (only used when USE_WAVELET_ATTENTION = True)
WAVELET_TYPE = 'db4'
WAVELET_LEVEL = 3
WAVELET_ATTENTION_REDUCTION = 2

# Training parameters (ORIGINAL - unchanged)
NUM_EPOCHS = 50
...
# Conditional import for wavelet
if USE_WAVELET_ATTENTION:
    try:
        ...
        print("   Disabling wavelet attention.")
...
# ============================================================================
# WAVELET ATTENTION MODULE (only used when USE_WAVELET_ATTENTION = True)
# ============================================================================
if USE_WAVELET_ATTENTION:
    class ChannelAttention(nn.Module):
        """
        Squeeze-and-excitation style channel attention over wavelet bands.
        Computes a scalar weight for each frequency band based on global temporal
        statistics of the band features.
        """
        def __init__(self, num_bands, reduction=2):
            super().__init__()
            self.fc1 = nn.Linear(num_bands, num_bands // reduction)
            self.fc2 = nn.Linear(num_bands // reduction, num_bands)
            self.activation = nn.ReLU()

        def forward(self, band_features):
            """
            band_features: (B, L, C) where C = num_bands (level+1)
            returns: (B, C) attention weights (softmax over bands)
            """
            B, L, C = band_features.shape
            pooled = band_features.mean(dim=1)          # (B, C)
            w = self.fc2(self.activation(self.fc1(pooled)))  # (B, C)
            w = torch.softmax(w, dim=-1)
            return w

    class WaveletAttentionFrontend(nn.Module):
        """
        Frequency-aware front-end with learnable channel attention over wavelet bands.
        Output size: (B, L, 2) – original signal + attention-weighted frequency feature.
        """
        def __init__(self, sequence_length=100, wavelet='db4', level=3, reduction=2):
            super().__init__()
            self.sequence_length = sequence_length
            self.wavelet = wavelet
            self.level = level
            self.num_bands = level + 1
            self.channel_attn = ChannelAttention(self.num_bands, reduction)
        ...
        def _wavelet_decompose(self, x):
            ...
            for b in range(B):
                ...
                for i, c in enumerate(coeffs):
                    rec = pywt.upcoef('a' if i == 0 else 'd', c, self.wavelet,
                                      level=self.level, take=len(signal))
                    bands_per_sample.append(rec[:L])
                level_features = np.stack(bands_per_sample, axis=-1)
                all_bands.append(level_features)

            band_features = torch.tensor(np.stack(all_bands, axis=0),
                                         dtype=dtype, device=device)
            return band_features

        def forward(self, x):
            band_features = self._wavelet_decompose(x)          # (B, L, C)
            w = self.channel_attn(band_features)                # (B, C)
            weighted_bands = band_features * w.unsqueeze(1)     # (B, L, C)
            fused = weighted_bands.sum(dim=-1, keepdim=True)    # (B, L, 1)
            enriched = torch.cat([x, fused], dim=-1)            # (B, L, 2)
            return enriched
...
# Iterative Attention Reasoner removed – no longer used.

class ImprovedxLSTMPredictor(nn.Module):
    """
    N-Layer Stacked xLSTM (without attention reasoner).
    Optional wavelet attention front-end (controlled by USE_WAVELET_ATTENTION).
    """
    def __init__(self, input_size=1, hidden_size=128, mem_dim=16, output_size=1, 
                 num_layers=4, attention_iterations=3):
        super().__init__()
        self.input_size = input_size
        self.hidden_size = hidden_size
        self.mem_dim = mem_dim
        self.num_layers = num_layers
        self.use_wavelet = USE_WAVELET_ATTENTION
        
        # Wavelet attention front-end (optional)
        if self.use_wavelet:
            self.wavelet_frontend = WaveletAttentionFrontend(
                sequence_length=SEQUENCE_LENGTH,
                wavelet=WAVELET_TYPE,
                level=WAVELET_LEVEL,
                reduction=WAVELET_ATTENTION_REDUCTION
            )
            # Front-end outputs 2 channels: original + fused frequency feature
            input_proj_size = 2
        else:
            # Original code path: single input channel
            input_proj_size = input_size
        
        self.input_proj = nn.Linear(input_proj_size, hidden_size)
        
        # N mLSTM layers
        self.mlstm_layers = nn.ModuleList([
            ImprovedmLSTMBlock(hidden_size if i == 0 else hidden_size // 2, 
                              hidden_size // 2, mem_dim=mem_dim)
    ...
    def forward(self, x):
        ...
        # Apply wavelet attention front-end if enabled
        if self.use_wavelet:
            x = self.wavelet_frontend(x)  # (B, L, 2)
        # If not using wavelet, x remains (B, L, 1) - original behavior
        ...
        for t in range(seq_len):
            ...
            for i in range(self.num_layers):
                x_proj, hidden_states[self.num_layers + i] = self.slstm_layers[i](
...
def save_checkpoint(epoch, model, optimizer, scheduler, train_losses, val_losses, 
                   val_r2_scores, best_val_loss, patience_counter, 
                   training_start_time, filename, args, 
    ...
    checkpoint = {
        ...
        'config': {
            ...
            'WAVELET_TYPE': WAVELET_TYPE if USE_WAVELET_ATTENTION else None,
            'WAVELET_LEVEL': WAVELET_LEVEL if USE_WAVELET_ATTENTION else None
        }
    }
...
def load_checkpoint(filename, model, optimizer, scheduler, device):
    if os.path.isfile(filename):
        ...
        return (checkpoint['epoch'], checkpoint['train_losses'], 
                checkpoint['val_losses'], checkpoint['val_r2_scores'],
                checkpoint['best_val_loss'], checkpoint['patience_counter'],
...
if __name__ == "__main__":
    ...
    model_variant = "WITH WAVELET ATTENTION" if USE_WAVELET_ATTENTION else "BASELINE (NO WAVELET)"
    
    print("\n" + "="*70)
    print(f" xLSTM VIBRATION PREDICTION - {NUM_LAYERS}-LAYER STACK")
    ...
    if USE_WAVELET_ATTENTION:
        print(f"  Wavelet Front-end: {WAVELET_TYPE} L{WAVELET_LEVEL} + Channel Attention")
        print(f"  Enriched Input Features: 2 (original + fused attention)")
    else:
        print(f"  Input Features: 1 (raw signal only)")
    ...
    start_epoch, train_losses, val_losses, val_r2_scores, best_val_loss, \
    patience_counter, training_start_time, loaded_feature_scaler, \
    loaded_target_scaler = load_checkpoint(
    ...
    for epoch in range(start_epoch, NUM_EPOCHS):
        ...
        save_checkpoint(
            ...
            val_r2_scores=val_r2_scores, best_val_loss=best_val_loss,
            patience_counter=patience_counter,
            training_start_time=training_start_time,
            filename=CHECKPOINT_FILE, args=args,
    ...
    if not args.deepspeed or args.local_rank == 0:
        ...
        model.load_state_dict(
            torch.load(model_path, map_location=map_device, weights_only=False)
        )
        ...
        if USE_WAVELET_ATTENTION:
            print(f"  Wavelet: {WAVELET_TYPE} L{WAVELET_LEVEL} + Channel Attention")
        print(f"{'='*70}\n")
        ...
        ax6.axis('off')
        wavelet_info = f"Wavelet: {WAVELET_TYPE} L{WAVELET_LEVEL} + Attention" if USE_WAVELET_ATTENTION else "No Wavelet"
        metrics_text = f"""
        PERFORMANCE SUMMARY
        ({model_variant})
        ...
        ax6.set_title(f'{NUM_LAYERS}-Layer xLSTM - {model_variant}', 
